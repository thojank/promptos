...
4. **[backend/db_models.py](backend/db_models.py)** — Datenbank-Schema
    **WICHTIG:** Alle Datenbanktabellen müssen IMMER mit `prompt_` beginnen (z.B. `prompt_character_profiles`, `prompt_scene_modules`, `prompt_stories`).
    - `prompt_character_profiles` — Wiederverwendbare Character-Templates
    - `prompt_scene_modules` — Austauschbare Szenen
    - `prompt_text_elements` — Text-Objekte für Bilder
    - `prompt_stories` — Container für Prompt-Sequenzen
    - `prompt_prompts` — Finale Kombinationen
    - `prompt_story_versions` — Versionskontrolle
...
1. Gehe zu [Google AI Studio](https://aistudio.google.com/app/apikeys)
2. Erstelle einen API-Key für Gemini
3. Speichere in `.env`:

```bash
# .env
GEMINI_API_KEY=dein_api_key_hier
DATABASE_URL=postgresql://user:password@localhost/zimage_turbo
```
...
### Design & UI-Spezifikationen

- **CSS Framework:** Tailwind CSS für schnelles, responsives Design.
- **Farb-Modus:** Light- und Dark-Mode Unterstützung von Anfang an einplanen.
- **Typografie:**
  - **Headlines:** 'Scope One' (Serif) - [Google Fonts Link](https://fonts.google.com/specimen/Scope+One)
  - **Body & Subheadlines:** 'IBM Plex Sans Condensed' (Sans Serif) - [Google Fonts Link](https://fonts.google.com/specimen/IBM+Plex+Sans+Condensed)
...
- [ ] `.env` mit GEMINI_API_KEY erstellen
- [ ] `python gemini_integration.py` testen (sollte Prompt generieren)
- [ ] `main.py` mit FastAPI-Endpoints schreiben
- [ ] Lokalen Server starten (`uvicorn backend.main:app --reload`)
- [ ] Frontend-Basis mit Next.js aufsetzen
- [ ] Erstes Input-Form für "Generate Prompt" bauen
